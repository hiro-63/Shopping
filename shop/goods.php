...
<div class="box" style="width: 100%; padding: 8px; font-size: 13px;">
    ...
    <div class="product-list"
        ...
        foreach ($products as $row) {
            ...
            echo '<img src="image/' . $id . '.jpg?' . date("YmdHis") . '" alt="' . $name . '" style="width: 100%; height: 80px; object-fit: cover; border-radius: 4px;">';
            echo '<h3 style="font-size: 13px; margin: 6px 0;">' . $name . '</h3>';
            echo '<p style="margin: 4px 0;">価格：¥' . $price . '</p>';
            echo '</div>';
        }
        ?>
    </div>
...
<form method="POST" action="" class="search-form-1">
    <label>
        <input type="text" name="text" placeholder="キーワードを入力"
               value="<?= isset($_POST['text']) ? htmlspecialchars($_POST['text'], ENT_QUOTES, 'UTF-8') : '' ?>">
    </label>
...
try {
    ...
    foreach ($sql as $row) {
        ...
        echo '<img src="image/' . $id . '.jpg?' . date("YmdHis") . '" alt="' . $name . '">';
        ...
        echo '<input type="hidden" name="price" value="' . (int)$row['price'] . '">';
        ...
        echo '<button type="button" class="favorite-star" onclick="toggleFavorite(this, ' . $id . ')">&#9734;</button>';
        echo '</div>';
